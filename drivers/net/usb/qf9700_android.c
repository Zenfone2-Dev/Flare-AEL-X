...
static void qf9700_android_mdio_write(struct net_device *netdev, int phy_id, int loc, int val)
{
	...
	netdev_dbg(dev->net,"qf9700_android_mdio_write() phy_id=0x%02x, loc=0x%02x, val=0x%04x",
	       phy_id, loc, val);
...
static void qf9700_android_set_multicast(struct net_device *net)
{
	...
	if (net->flags & IFF_PROMISC) {
		...
		//for (i = 0; i < netdev_mc_count(net); i++, mc_list = mc_list->next) 
...
static const struct net_device_ops qf9700_netdev_ops = {
	...
	.ndo_do_ioctl		    = qf9700_android_ioctl,
	.ndo_set_rx_mode = qf9700_android_set_multicast,
};
...
static int qf9700_android_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	//dev->net->do_ioctl = qf9700_android_ioctl;
	//dev->net->set_multicast_list = qf9700_android_set_multicast;
    
    dev->net->netdev_ops = &qf9700_netdev_ops;
	dev->net->ethtool_ops = &qf9700_android_ethtool_ops;
	dev->net->hard_header_len += QF_TX_OVERHEAD;
	dev->hard_mtu = dev->net->mtu + dev->net->hard_header_len;
	dev->rx_urb_size = dev->net->mtu + ETH_HLEN + QF_RX_OVERHEAD;

	dev->mii.dev = dev->net;
	dev->mii.mdio_read = qf9700_android_mdio_read;
	dev->mii.mdio_write = qf9700_android_mdio_write;
	...
	mdelay(20 * 1000);	// at least 10ms, here 20ms for safe
	qf_write_reg(dev, PRR, 0);
	mdelay(2 * 1000);	// at least 1ms, here 2ms for reading right register

	/* receive broadcast packets */
	qf9700_android_set_multicast(dev->net);

	qf9700_android_mdio_write(dev->net, dev->mii.phy_id, MII_BMCR, BMCR_RESET);
	qf9700_android_mdio_write(dev->net, dev->mii.phy_id, MII_ADVERTISE, ADVERTISE_ALL | ADVERTISE_CSMA | ADVERTISE_PAUSE_CAP);
